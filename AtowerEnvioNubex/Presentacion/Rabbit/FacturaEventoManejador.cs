...
namespace AtowerEnvioNubex.Presentacion.Rabbit
{
    public class FacturaEventoManejador : IEventoManejador<FacturaEventoQueue>
    {
        ...
        public async Task Handle(FacturaEventoQueue @event)
        {
            ...
            var idCompanyNubex = @event.IdCompañiaNubex;
            var idCompanyAtower = @event.IdCompañiaAtower;
            var response = await _envioFacturaNubexDian.EnviarFacturaNubexDian(@event.Factura, idCompanyNubex);
