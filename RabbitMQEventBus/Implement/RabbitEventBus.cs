...
namespace RabbitMQEventBus.Implement
{
    public class RabbitEventBus : IRabbitEventBus
    {
        ...
        {
            var factory = new ConnectionFactory() { HostName = "rabbit-alejo-web" };
            ...
            where TH : IEventoManejador<T>
        {
            ...
            {
                throw new ArgumentException($"El manejador {manejadorEventoTipo.Name} fuen registrado anteriormente por {eventoNombre}");
            }
        ...
        private async Task Consumer_Delegate(object sender, BasicDeliverEventArgs e)
        {
            ...
            var menssage = Encoding.UTF8.GetString(e.Body.ToArray());
            ...
            try
            {
                if (_manejadores.ContainsKey(nombreEvento))
                {
                    ...
                    {
                        var susbcriptions = _manejadores[nombreEvento];
                        foreach (var sb in susbcriptions)
                        {
                            ...
                            var eventoDS = JsonConvert.DeserializeObject(menssage, tipoEvento);
